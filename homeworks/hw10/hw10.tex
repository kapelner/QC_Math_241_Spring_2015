...
\title{MATH 241 Spring 2015 Homework \#10}
...
\date{Due 5PM, Tuesday, May 5, 2015 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
Reading is still \textit{required}. But for this homework set, read about moment generating functions and their properties and read about the central limit theorem.
...
Prove that the mgf for a standard Cauchy r.v. does not exist. This is really not hard but it looks menacing. Do on a separate piece of paper. \spc{2}
...
\easysubproblem Show that $C_n = \frac{Z_1}{\sqrt{n}} + \frac{Z_2}{\sqrt{n}} + \ldots + \frac{Z_n}{\sqrt{n}}$.  \spc{2}
...
I will consider this to be called the \qu{bathtub function.} This is not a real brand name distribution. I just made it up and it won't be on the test. Write about why you think I called it the \qu{bathtub function.} Does this look like a bell curve at all?? \spc{2}
...
\easysubproblem Let's sum up what we've learned in this problem. The central limit states as $n \rightarrow \infty$ (i.e. it gets big) then $C_n$ becomes a standard normal. Does it become a standard normal at different rates depending on the distribution of the r.v. being sampled? Yes/no is fine.

\end{enumerate}

\problem Now that we've now proven the CLT, we are going to use it a little bit.
...
\beqn
\frac{T_n - n\mu}{\sqrt{n}\sigma} \sim \stdnormnot
\eeqn 

which follows from the last question instantaneously.\spc{2}
