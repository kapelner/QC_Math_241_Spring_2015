...
\iftoggle{professormode}{
\paragraph{The Power of the CLT} In the last homework we proved the CLT, now we examine its power. We can do statistical inference and statistical testing!\\ \\
} 
...
\easysubproblem Because $\Xbar$'s support is naturally bounded in the Bernoulli experiment case (see questions a and b of this problem), we give it a special notation: $\Phat$. Write the definition of $\Phat$ mathematically.  \spc{1}
...
\intermediatesubproblem Use your previous answers and assume $n$ is big enough for the CLT to \qu{kick in} as well as what you know about normal distributions shifted and scaled to find the distribution of $\Phat_n$.  \spc{2}

\hardsubproblem Write the PDF of $\Phat_n$. This will force you to understand (1) the generalized density for $\normnot{\mu}{\sigsq}$ which is in the notes but has never been previously asked on a homework assignment, (2) substitutions for the mean and variance and (3) what the free variable is.  \spc{8}
...
\easysubproblem Illustrate an example of a $\phat$ with the same half-width to the left and right interval that does \textit{not} cover $p$ in your picture of 1(k). No need to write anything here. 
...
\easysubproblem Compute a 99\% CI for $p$, the true proportion of blue M\&M's using the same data as in (e). Assume all of the CI assumptions are correct. I gave you this $z_{\overtwo{\alpha}}$ value in class. \spc{2}
...
\easysubproblem For the interval you created in (e) provide two valid classical interpretations for it \textit{in English}.  \spc{4}

\easysubproblem For the interval you created in (e) write the non-classical interpretation \textit{in English} --- this is the interpretation you really want to be able to say...  \spc{3}
...
\hardsubproblem Now let $n=4,247,000$ (the number of American newborns in the year 2008) and use the same $\alpha =1\%$. You get 2,173,000 males. Write the hypotheses, find the acceptance region and find the result and interpret it. \spc{9}
...
\intermediatesubproblem At $\alpha$ = 2.5\%, what is the acceptance region? \spc{4}
...
\hardsubproblem Your answer changed! Why is it a bad idea to run the test like in (f) / why is it a better idea to run the test like in (e)? \spc{5}

\extracreditsubproblem If the driver's true proportion / propensity of bad rides is $6\%$, what is the probability of a type II error when running the test in (e)?
